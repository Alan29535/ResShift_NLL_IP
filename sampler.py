...
class BaseSampler:
    ...
    def build_model(self):
        ...
        model = util_common.instantiate_from_config(self.configs.model).to(self.device)
...
class ResShiftSampler(BaseSampler):
    ...
    def inference(self, in_path, out_path, mask_path=None, mask_back=True, bs=1, noise_repeat=False):
        ...
        def _process_per_image(im_lq_tensor, mask=None):
            ...
            context = torch.cuda.amp.autocast if self.use_amp and self.device.type == 'cuda' else nullcontext
        ...
        if in_path.is_dir():
            ...
            for data in dataloader:
                ...
                if micro_data['lq'].shape[0] > 0:
                    results = _process_per_image(
                            micro_data['lq'].to(self.device),
                            mask=micro_data['mask'].to(self.device) if 'mask' in micro_data else None,
            ...
            if mask_path is not None:
                ...
                im_mask_tensor = util_image.img2tensor(im_mask).to(self.device)              # 1 x c x h x w
        ...
        self.write_log(f"Processing done on {self.device.type}, enjoy the results in {str(out_path)}")
